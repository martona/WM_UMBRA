...
//
// WH_CALLWNDPROCRET is thread-local, and a hook proc that lives in the EXE (not
// a DLL) may only be installed on the CALLING thread — installing it on another
// thread fails with ERROR_INVALID_PARAMETER. So each thread hooks ITSELF: the
...
namespace
{
    ...
    {
        const HHOOK hk = ::SetWindowsHookExW(WH_CALLWNDPROCRET, CallWndRetProc,
            nullptr, ::GetCurrentThreadId());
        wchar_t line[160];
        ::StringCchPrintfW(line, ARRAYSIZE(line), L"SELFHOOK tid=%lu  hook=%p  err=%lu\r\n",
            ::GetCurrentThreadId(), reinterpret_cast<void*>(hk), hk ? 0UL : ::GetLastError());
        Dbg(line);
    }
